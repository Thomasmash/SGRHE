          ...
          <div class="content-wrapper">
            ...
            <section class="content-header">
              <div class="container-fluid">
                <div class="row mb-2">
                  <div class="col-sm-6">
                    <h1>Secções / Departamentos</h1>
                  </div>
                  <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                      <li class="breadcrumb-item"><a href="#">Home</a></li>
                      <li class="breadcrumb-item active">Secções / Departamentos </li>
                    </ol>
                  </div>
                </div>
              </div><!-- /.container-fluid -->
                    ...
                    <div style="padding:10px; border-radius:5px; " class="col-12">
                      <div style="background-color: #ffffff;" class="card card-primary">
                          ...
                          <div class="card-header">
                                <h3 class="card-title">Secções e Departamentos</h3>  
                          </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                          <table id="example1" class="table table-bordered table-striped">
                            ...
                            @foreach ($dados as $seccao)
                                          <tr>
                                              <td>{{ $loop->index+1 }}</td>
                                              <td>{{ $seccao->codNome }}</td>
                                              <td>{{ $seccao->designacao }}</td>
                                              <td>{{ $seccao-> idChefe }}</td>
                                              <td>
                                                <form action="{{ route('cargos.form', ['id' => $seccao->id]) }}" method="POST" style="display: inline;">
                                                  @csrf
                                                  @method('PUT')
                                                    <button type="submit" class="btn btn-warning w-100 m-1">Editar</button>
                                                </form>
                                                <form action="{{ route('eliminar.objecto') }}" method="POST" id="deleteForm{{ $seccao->id }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <input type="hidden" name="id" value="{{ $seccao->id }}">
                                                    <input type="hidden" name="categoria" value="Seccao">
